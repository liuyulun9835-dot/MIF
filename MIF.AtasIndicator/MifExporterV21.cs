...
{
    /// <summary>
    /// MIF ATAS导出器 V21 - V20基础上增强Cluster诊断能力
    ///
    /// V18关键更新：
    ...
    /// - 固定维度 - 保证数据维度一致性
    /// </summary>
    [DisplayName("MIF Exporter V21")]
    public class MifExporterV21 : Indicator
    {
        #region 配置参数
        ...
        private static bool _candleProbed = false;

        #endregion

        public MifExporterV21()
        {
            Name = "MIF Exporter V21";
            ...
            try
            {
                ...
                if (_alivePath != null)
                {
                    ...
                    File.AppendAllText(_alivePath,
                        ...
                        $"V21 Features: Cluster diagnostics enhanced (ISupportedPriceInfo + fallback logging)\n" +
                        $"{new string('=', 80)}\n\n");

                    File.AppendAllText(_alivePath,
                        "[INIT] Will probe candle structure on first bar\n");
                }
            }
            catch (Exception ex)
            {
        ...
        {
            IndicatorCandle? probedCandle = null;

            if (!_candleProbed && _alivePath != null)
            {
                _candleProbed = true;
                probedCandle = GetCandle(bar);

                if (probedCandle != null)
                {
                    try
                    {
                        var candleType = probedCandle.GetType();
                        File.AppendAllText(_alivePath,
                            $"\n[PROBE] Candle Structure Deep Inspection:\n" +
                            $"  Type: {candleType.FullName}\n" +
                            $"  Assembly: {candleType.Assembly.FullName}\n");

                        var allProps = candleType.GetProperties(BindingFlags.Public | BindingFlags.Instance);
                        File.AppendAllText(_alivePath,
                            $"  Public Properties ({allProps.Length}):\n");

                        foreach (var prop in allProps.Take(30))
                        {
                            try
                            {
                                var val = prop.GetValue(probedCandle);
                                var valStr = val?.ToString() ?? "NULL";
                                if (valStr.Length > 50) valStr = valStr.Substring(0, 50) + "...";
                                File.AppendAllText(_alivePath,
                                    $"    {prop.Name} ({prop.PropertyType.Name}): {valStr}\n");
                            }
                            catch (Exception ex)
                            {
                                File.AppendAllText(_alivePath,
                        ...
                        if (allProps.Length > 30)
                        {
                            File.AppendAllText(_alivePath,
                                $"    ... ({allProps.Length - 30} more properties omitted)\n");
                        }

                        var allMethods = candleType
                    ...
                    {
                        File.AppendAllText(_alivePath,
                            $"[PROBE-ERROR] Candle inspection failed: {ex.Message}\n");
                    }
                }
            }
            ...
            var candle = probedCandle ?? GetCandle(bar);
            ...
            // === 周期性触发详细诊断 (每256个bar一次) ===
            bool enableDetailedDiag = (bar & 255) == 0;

            if (enableDetailedDiag && _alivePath != null)
            {
                File.AppendAllText(_alivePath,
                    $"\n{new string('=', 60)}\n" +
                    $"[PERIODIC-DIAG] Bar {bar} at {DateTime.UtcNow:o}\n" +
                    $"{new string('=', 60)}\n");
            }
            ...
            var rec = new
            {
                header = new
                {
                    ...
                    version = "mif.v21.0",
                    exporter = "MIF.AtasIndicator.V21",
        ...
        protected override void OnDispose()
        {
            ...
            try
            {
                ...
                {
                    File.AppendAllText(_alivePath,
                        $"\n[V21-BACKTRACK] Starting full history scan from bar 0 to {CurrentBar - 1}...\n");
                }
                ...
                {
                    File.AppendAllText(_alivePath,
                        $"[V21-BACKTRACK] Completed. Recovered {backtrackCount} missing bars.\n\n");
                }
            }
            catch (Exception ex)
            {
        ...
        private bool TryExtractClusterLevels(IndicatorCandle candle, out PriceLevelDTO[] levels)
        {
            ...
            bool diagLogEnabled = _alivePath != null;
            ...
            try
            {
                ...
                {
                    File.AppendAllText(_alivePath!,
                        "[DIAG] TryExtractClusterLevels START\n" +
                        $"  Candle Type: {candle.GetType().FullName}\n" +
                        $"  Is ISupportedPriceInfo: {candle is ISupportedPriceInfo}\n");
                }
                ...
                if (candle is ISupportedPriceInfo priceInfo)
                {
                    ...
                    if (diagLogEnabled)
                    {
                        string allLevelsType = allLevels?.GetType()?.FullName ?? "NULL";
                        int allLevelsCount = 0;

                        if (allLevels is IEnumerable enumForCount)
                        {
                            foreach (var _ in enumForCount)
                            {
                                allLevelsCount++;
                            }
                        }
                        ...
                        File.AppendAllText(_alivePath!,
                            $"  GetAllPriceLevels() Type: {allLevelsType}\n" +
                            $"  GetAllPriceLevels() Count: {allLevelsCount}\n");
                    }
                    ...
                    if (allLevels is IEnumerable enumerableLevels)
                    {
                        ...
                        foreach (var pl in enumerableLevels)
                        {
                            ...
                            if (diagLogEnabled && rawItemCount <= 3)
                            {
                                var plType = pl.GetType();
                                var plProps = plType.GetProperties(BindingFlags.Public | BindingFlags.Instance)
                                    .Select(p => $"{p.Name}={p.GetValue(pl)}")
                                    .ToArray();

                                File.AppendAllText(_alivePath!,
                                    $"  Item[{rawItemCount}] Type: {plType.FullName}\n" +
                                    $"  Item[{rawItemCount}] Props: {string.Join(", ", plProps)}\n");
                            }
                        ...
                        {
                            File.AppendAllText(_alivePath!,
                                "  ISupportedPriceInfo Path Results:\n" +
                                $"    Raw items: {rawItemCount}\n" +
                                $"    Null items: {nullItemCount}\n" +
                                $"    Zero items: {zeroItemCount}\n" +
                                $"    Valid levels: {validLevels.Count}\n");
                        }
                        ...
                        if (validLevels.Count > 0)
                        {
                            ...
                            {
                                File.AppendAllText(_alivePath!,
                                    $"[DIAG] SUCCESS via ISupportedPriceInfo: {levels.Length} levels\n\n");
                            }
                    ...
                    {
                        File.AppendAllText(_alivePath!,
                            "  GetAllPriceLevels() is NOT IEnumerable\n");
                    }
                }
                ...
                {
                    File.AppendAllText(_alivePath!,
                        "  Trying Fallback path (property access)...\n");
                }
                ...
                foreach (var propName in candidates)
                {
                    ...
                    {
                        File.AppendAllText(_alivePath!,
                            $"  Checking property '{propName}': {(prop == null ? "NOT FOUND" : "EXISTS")}\n");
                    }
                    ...
                    if (diagLogEnabled)
                    {
                        string containerType = container?.GetType()?.FullName ?? "NULL";
                        File.AppendAllText(_alivePath!,
                            $"    Property '{propName}' Type: {containerType}\n");
                    }
                    ...
                    if (container is IEnumerable enumerable)
                    {
                        ...
                        foreach (var item in enumerable)
                        {
                            ...
                            if (diagLogEnabled && fallbackItemCount <= 3)
                            {
                                ...
                                File.AppendAllText(_alivePath!,
                                    $"    Fallback Item[{fallbackItemCount}] Type: {itemType.FullName}\n" +
                                    $"    Fallback Item[{fallbackItemCount}] Props: {string.Join(", ", itemProps)}\n");
                            }
                        ...
                        {
                            File.AppendAllText(_alivePath!,
                                $"    Fallback '{propName}' items: {fallbackItemCount}\n" +
                                $"    Fallback valid levels: {validLevels.Count}\n");
                        }
                        ...
                        if (validLevels.Count > 0)
                        {
                            ...
                            {
                                File.AppendAllText(_alivePath!,
                                    $"[DIAG] SUCCESS via Fallback '{propName}': {levels.Length} levels\n\n");
                            }
                ...
                {
                    File.AppendAllText(_alivePath!,
                        "[DIAG] FAILED: No valid cluster data found\n\n");
                }
            }
            catch (Exception ex)
            {
                ...
                {
                    File.AppendAllText(_alivePath,
                        $"[ERROR] TryExtractClusterLevels exception: {ex.Message}\n" +
