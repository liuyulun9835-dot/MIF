...
理论定义（方案A/B/C）中 I(Γ) ∈ [0, 1]。
但Strategy t2中的操作性定义：
I(Γ) = EMA_9[(z_comp + z_POC + z_u) / 3]
是三个z-score的均值的EMA，值域无界。

两者不矛盾：理论定义描述归一化后的Fisher信息，
操作性定义是其z-score代理。

在使用时：I_critical应设为I(Γ)历史分布的分位数
（如quantile(I(Γ), 0.60)），而非绝对值。
这与ADR-002（分位数阈值）一致。
...
边界防护：
若 ||Γ(t)|| = 0（所有层所有侧volume均为0），
该bar标记为异常（data_quality_flag），
不参与ω_t计算和后续判别。
```
...
> **计算可行性注（v2.4.1）**：I_em的联合熵H[ε_buy, ε_sell]需要估计二维联合分布。每个DOM快照仅提供20个paired样本（20层），若使用5×5=25 bins的histogram，严重欠采样导致MI估计偏高且噪声大。
>
> **Phase 1替代方案**：使用买卖侧向量的余弦相似度 `S_bs = cos_sim(ε_buy[0:20], ε_sell[0:20])` 替代I_em。S_bs在20维空间上精确可算（无估计噪声），物理含义高度重叠（都度量买卖侧分布的对齐程度）。代价：丢失非线性关联信息，但20样本下本来也估不出非线性关联。
>
> **切换条件**：当多bar pooled estimation可行（如积累100+ bar后用滑动窗口内的pooled samples估计联合分布），可回退到真正的I_em。切换前需验证I_em与S_bs的rank correlation > 0.8。
...
Component 3 - 相位相干度：
ω_p = α × S_bs + (1-α) × [(1+cos φ)/2]
α ∈ [0,1]：权重

其中：
S_bs = cos_sim(ε_buy[0:N], ε_sell[0:N])
    = ⟨ε_buy, ε_sell⟩ / (||ε_buy|| × ||ε_sell||)
由于ε ≥ 0，S_bs ∈ [0, 1]

Phase 1使用S_bs替代I_em（原因见Level 2计算可行性注）
当I_em的可靠估计可用时，可替换回：
...
Phase 1实现（I_em不可靠时）：
div J = ΔH_m/Δt - ΔS_bs/Δt

离散形式：
div J(t) = [H_m(t) - H_m(t-Δt)]/Δt - [I_em(t) - I_em(t-Δt)]/Δt

物理意义保持一致：
div J < 0 = H_m降（能量集中）且/或 S_bs升（买卖对齐）→ 信息凝聚
div J > 0 = 反向 → 信息扩散
...
ε₁、ε₂、ε_critical均为对应变量历史分布的分位数，
不是硬编码绝对值。

建议初始值：
ε₁ = quantile(|dΩ/dt|, 0.60)
ε₂ = quantile(|div J|, 0.60)
ε_critical = quantile(|dΩ/dt|, 0.95)
...
> **操作化状态声明（v2.4.1）**：本节描述的概率框架是MIF的**研究纲领**，不是当前可执行的工程规范。
>
> - P_i(t)（涌现事件概率分布）当前不可计算——需要Γ→P的映射模型，该模型不存在。
> - 六种触发假设（A-F）是待验证的研究假设，不是已实现的计算模块。
> - Regime Transition的四个触发条件中，两个依赖P_i（不可计算），一个依赖S_ext（DOM不可观测），一个（能量积分）可计算但阈值待标定。
> - 三种演化方程的参数全部未知。
>
> Strategy t2通过以下代理绕开了本节的不可计算项：
> - P_i(t) → IS/UIS二值判定 + confidence连续评分
> - dP_i/dt → dΩ/dt + Hawkes n
> - S_ext → 不处理（承认为理论边界外的风险）
> - E_barrier → Hawkes n ≥ 1 作为transition确认的代理
>
> 本节的价值在于为后续研究（如从Γ直接学习P_i的映射模型）提供理论方向。
...
> **操作化状态**：假设A、B可直接实现（仅需事件元序列和dΩ/dt）；假设C、D、E需要额外定义或标注数据；假设F依赖DOM外部信息，不可从DOM数据实现。验证需先标注historical regime transitions，再做模型选择（BIC）。这是一个离线研究任务，不是实时计算任务。
...
> **操作化状态**：上述四个触发条件中：
> - 条件1（概率梯度）和条件2（涨落振幅）：依赖P_i(t)，当前不可计算。
> - 条件3（能量窗口）：积分∫|dΩ/dt|²dt可计算，但E_barrier阈值需要标注数据标定。
> - 条件4（外部冲击）：S_ext按定义不在DOM数据中，不可观测。
>
> Strategy t2使用Hawkes n≥1作为regime transition的代理确认，绕开了这四个条件。
...
上述 Ω→I(Γ)→Hawkes 的顺序是当前工程默认值，不是理论必然。

信息论中已知：对同一组数据的分解顺序会影响计算有界观察者
能提取的结构信息量。这意味着先做Ω门控再触发Hawkes，
和先做Hawkes检测再用n调制Ω的解读权重，
可能提取到不同量的结构信息。

当前顺序的设计逻辑是"从宏观到微观逐层筛选"：
先确认环境可读（Ω），再确认结构存在（I(Γ)），
最后确认突破自持（n）。

该顺序的潜在盲点：当事件层已出现自激信号（n≥1）
但结构层尚未完全成型（Ω仍在门控阈值以下）时，
当前流水线会在Step 1拒绝，可能遗漏IS的早期信号。

验证计划：在积累足够数据后，统计"Step 1拒绝但n≥1"
的事件频率和后续收益，据此决定是否调整顺序或改为并行判定。
...
> **底数约定**：全文Shannon熵使用log₂（单位：bit）。H_m中的log和ω_s中的log(n_levels)须使用相同底数。
...
ω_s = 1 - H_m/H_max
ω_t = Σ w_k ⟨Γ(t), Γ(t-k)⟩
ω_p = α×S_bs + (1-α)×(1+cos φ)/2  # Phase 1; 原式: α(I_em/log n)
```

**动力学**：
```
div J = ∂H_m/∂t - ∂S_bs/∂t  # Phase 1; 原式: ∂H_m/∂t - ∂I_em/∂t
dΩ/dt = [Ω(t) - Ω(t-Δt)]/Δt
```
