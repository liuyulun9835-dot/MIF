...
## ADR-004: 在开/加/平仓逻辑中引入MSI（多尺度追踪）

**日期**: 2025-11-12  
**状态**: ✅ Accepted  
**决策者**: Sevi + GPT-5 Thinking

### 上下文
单一时间尺度的信号稳定性不足，易被假趋势诱导；需要在**不牺牲交易频率**的前提下，自适应结构变化：小结构早进早退，大结构长持慢退。

### 决策
在 `MIF_Strategy_t1.md` 的开仓、加仓、平仓段落中，叠加 MSI 三要素：
1) `C_ms` 跨尺度一致性；2) `ΔΩ_multi` 延续速率；3) `MSI_weight` 置信缩放。
保持原有门槛不变，仅作**上层门控与缩放**。

### 理由
1. 放大肥尾：结构多尺度共振时延长持有、允许加仓；  
2. 减少磨损：短期扰动不退出，仅在**跨尺度退相干**时离场；  
3. 自适应：小结构快进快退，大结构慢进慢退，仓位即置信度。

### 后果
**正面**：假突破显著减少；收益分布更胖尾；退出更从容。  
**负面**：需要维护一个最小多尺度集合（如 {5m,15m,30m}）；计算略增。  

### 替代方案
A. 仅提高单尺度阈值（简单但仍被噪音误导）  
B. 机器学习评分（复杂度与过拟合风险不合要求）

### 验证方法
- A/B：启用MSI vs 关闭MSI 的夏普、胜率、回撤差异  
- 事件回放：MSI 触发的退出是否显著优于价格止损  
- 证伪：若样本外 MSI 不优于基线 → 可回退

### 实施计划
Phase-1：插入文档逻辑与最简实现（本次提交）  
Phase-2：回测评估阈值与权重  
Phase-3：再讨论自适应窗口替代固定窗口
...
## ADR-005: MSI动态响应理论（替代固定常数假设）【⚠️ 需要验证】
...
**1. 理论一致性**
- 边际均衡是经济学基本原理
- 解释了为什么观察到4-6 bar（典型值，非常数）
- 符合背景独立性（S是相对度量）

**2. 动态适应性**
```
强趋势(S>0.7): T* = 6-10 bars
中趋势(0.3<S<0.7): T* = 4-6 bars
弱趋势(S<0.3): T* = 2-4 bars
```

**3. 实时可求解**
- 使用条件期望：E[T*|Ω, E, dΩ/dt, 历史统计]
- 边际分析：if ∂Alpha/∂t > ∂Cost/∂t → 继续持有
- 不需要未来函数
...
**则**: 回退到固定窗口，或简化为2-3档分层（强/中/弱）

### 决策时间
2025-11-27: 根据Phase 2验证结果决定是否采用
