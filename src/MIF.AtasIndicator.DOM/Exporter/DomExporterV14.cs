    ...
    public class MifExporterV14F : Indicator
    {
        ...
        private void FlushBuffer(DateTime barTime)
        {
            ...
            try
            {
                ...
                var baseDirectory = Path.GetDirectoryName(_outPath)!;
                JsonlWriter.AppendRecords(baseDirectory, "bars", utcTime, _recordBuffer);
                _recordBuffer.Clear();
            }
            catch (Exception ex)
            {
        ...
        {
            FileLogger.LogError(_alivePath, message, ex);
        }
